...
PIXTYPE sigmaR, sigmaCTheta, sigmaRTheta, R, beta; //domain variance for the two filters: sigmaC, sigmaCTheta
	...
	levX=(int) (log10(pSrcImg->getXsize())/log10(2));
	levY=(int) (log10(pSrcImg->getYsize())/log10(2));
	if(levX < levY)
		levelMax = levX+1;
	else
		levelMax = levY+1;
		...
		for(j=0; j < jmax; j++) {
			for(i=0; i < imax; i++) {
				if( lev == 0) { //stack level 0 is the magnitude of the gradients of original image
					tmp = (PIXTYPE) sqrt( pX->get(i,j)*pX->get(i,j) + pY->get(i,j)*pY->get(i,j) );
					if(maxGrad < tmp)
						maxGrad = tmp;
					if(minGrad > tmp)
						minGrad = tmp;
		...
		for(j=0; j<jmax; j++) { //Y scaline
			...
			for(m = -halfSize; m<=halfSize; m++) {
				for (n = -halfSize; n<=halfSize; n++) {
					...
					if( (i+m) >= 0 && (i+m) <imax && (j+n) >=0 &&(j+n) < jmax) {
						g1 = (PIXTYPE) (pow( pX->get(i+m,j+n),2.0) + pow(pY->get(i+m,j+n),2.0) );
						g2 = (PIXTYPE) (pow( pX->get(i,j),2.0) + pow(pY->get(i,j),2.0) );
...
void Raw2D::sizer(int ixsize, int iysize) {
    	...
    	y = new PIXTYPE[ixsize*iysize];	// & allocate memory.
