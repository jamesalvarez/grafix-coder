...
void DialogTestSmoothParameters::fncGetSubmat(uword displayStartIndex, uword displayStopIndex) {
    ...
    uword stopIndex = qMin(displayStopIndex, p_original_roughM->n_rows - 1);
    uword startIndex = qMax(displayStartIndex, (uword)0);

    if (startIndex >= stopIndex) return;

    this->_displayStartIndex = startIndex;
    this->_displayStopIndex = stopIndex;
    this->_roughM = p_original_roughM->rows(startIndex, stopIndex);
}
